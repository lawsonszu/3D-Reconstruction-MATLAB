%% HiLo + 平方法 轴向扫描三维重建（整理版）
% 目标：在"信息充足区域"尽量还原真实光滑形貌（减少坑洼/同心圈）
% 核心流程：
%   Step1 读取并排序Z层数据
%   Step2 每层：平方法对比度图 -> HiLo光学切片 -> log融合，构建轴向响应体 vol
...
for k = 1:num_layers
    ...
    % 平方法：两个正交方向的结合（Frobenius范数）
    I0 = hypot(dV, dH);         % sqrt(dV^2 + dH^2)
    I0 = I0 - mean(I0(:));      % 零均值处理
    CI = imgaussfilt(I0, sigma_xy);
...
% 确保数据是实数且有限
peak_strength_valid = real(peak_strength);
peak_strength_valid(~isfinite(peak_strength_valid)) = 0;

% 用简单的归一化替代 mat2gray（避免 imlincomb 报错）
ps_min = min(peak_strength_valid(:));
ps_max = max(peak_strength_valid(:));
if ps_max > ps_min
    conf = (peak_strength_valid - ps_min) / (ps_max - ps_min);
else
    conf = ones(size(peak_strength_valid));
end
