...
for k = 1:num_layers
    ...
    % ---- Step2.1：SPQT（螺旋相位变换）计算对比度图（FV思想）----
    dV = I_V - I_UI;
    dH = I_H - I_UI;
    CI = 0.5 * (SPQT(dV) + SPQT(dH));
...
%% ====================== 函数区 ======================
function SPQT_edge = SPQT(img)
% SPQT：螺旋相位变换（Spiral Phase Quadrature Tensor）
% 用于焦点评估的对比度算子
    [rows, cols] = size(img);
    
    % 构建螺旋相位内核（frequency domain）
    [U, V] = meshgrid(-floor(cols/2):ceil(cols/2)-1, -floor(rows/2):ceil(rows/2)-1);
    R = sqrt(U.^2 + V.^2);
    
    % H = exp(i*theta) = (U + i*V) / R
    H = (U + 1i*V) ./ (R + eps);   
    H(R==0) = 0;   % DC分量置0
